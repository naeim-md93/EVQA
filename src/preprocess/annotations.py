from src.utils.pyutils import save_file
...
def filter_by_k_frequent_answers(data, answers):
    data = [d for d in data if d['answer'] in answers]
    return data


def get_answers_dictionary(answers, save_path, idx2ans_save_name, ans2idx_save_name):
    ...
    save_file(data=idx_to_answer, path=save_path, file_name=idx2ans_save_name, file_type='pickle')
    save_file(data=answer_to_idx, path=save_path, file_name=ans2idx_save_name, file_type='pickle')
